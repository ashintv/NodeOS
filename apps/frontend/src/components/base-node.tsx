...
export const BaseNode = forwardRef<HTMLDivElement, HTMLAttributes<HTMLDivElement>>(({ className, ...props }, ref) => (
	<div
		...
		className={cn(
			...
			// which compiles down to a div with the class `xy-flow__node`.
			// When a node is selected, the class `selected` is added to the
			// `xy-flow__node` element. This allows us to style the node when it
			// is selected, using Tailwind's `&` selector.
			"[.xy-flow\\_\\_node.selected_&]:border-muted-foreground",
			"[.xy-flow\\_\\_node.selected_&]:shadow-lg",
			className
		)}
		tabIndex={0}
		{...props}
